...
test('Shift is faster than walking; the exact value is a tuning knob, not a contract', () => {
  expect(RUN_SPEED).toBeGreaterThan(WALK_SPEED)
})

test('running into a wall on a hitchy frame still stops at the radius', () => {
  // A 1 m move in one frame — whatever RUN_SPEED is, dt is chosen to make the
  // move a metre — is more than a wall is thick plus the collision radius. Taken
  // as one step the visitor lands on the far side and the collider pushes them
  // the wrong way; the integrator has to sub-step so no single move skips a wall.
  const wall: Wall = { x1: -4, z1: 2, x2: 4, z2: 2, y0: 0, y1: 4 }
  let s = at(0, 1)
  for (let i = 0; i < 10; i++) s = integrate(s, keys({ forward: true, run: true }), 1 / RUN_SPEED, [wall])
  expect(s.z).toBeCloseTo(2 - COLLISION_RADIUS, 9)
})

...
vec3 get_value(sampler2D s, float i) {
  float dim = 512.;
  float off = 1. / (dim * 2.);
  float x = fract(i / dim);
  float y = floor(i / dim) / dim;
  return vec3(texture(s, vec2(x,y) + off)).xyz;
}

void main() {
  vTexCoord = aTexCoord;

  float id = floor(float(gl_VertexID) / 3.0);
  index = get_value(indexMap, id).x;
...
float univ_rand(vec2 p) {
  return texture(randvals, fract(p)).x;
}

void main() {
  vec2 uv = vTexCoord;
  //uv.y = 1.0 - uv.y;
  vec4 pixel = texture(imgTex,fract(uv + (univ_rand(vec2(index))-0.5)*0.2));
  fragColor = pixel;
}`;

const sedi_vert = `#version 300 es
#ifdef GL_ES
precision mediump float;
#endif
...
const sedi_frag = `#version 300 es
precision mediump float;
...
void main() {
  ...
  if (rand(uv+seed+723.) < agedProb) {
    loc = margin_wrap(loc + vec2(texture(imgTex, margin_wrap(loc+univ_rand(uv+seed)*0.1))) );
  }

  // drawn effect
  loc = drawn(loc);
  vec4 pixel = vec4(vec3(texture(imgTex, loc)), 1.); // fixes darkness issue
  vec4 orig = vec4(vec3(texture(imgTex, origuv)), 1.);
  ...
  if (rgbFilter > 0.) {
    ...
    pixel = vec4(
      texture(imgTex,loc).x, 
      texture(imgTex,fract(loc+goff)).y, 
      texture(imgTex,fract(loc+boff)).z, 
      1.
    );
  }
  
  // visualize the noise
  //float s = texture(randvals, origuv).x;
  //pixel = vec4(s, s, s, 1.);
  ...
  if ((origuv.x < limit) || (origuv.x > 1.-limit) || (origuv.y < limit) || (origuv.y > 1.-limit)) {
    ...
    fragColor = pixel + (rand(uv)-0.5) * 0.25;
    //fragColor = pixel + snoise(uv*1000.) * 0.4;
  }  
}`;
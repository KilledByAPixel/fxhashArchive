let overWriteHash = sha3_256(fxhash);
// let overWriteHash = sha3_256(
//   "ooUGELCJBShrHvk3owttDdCDuw7HG1Lq919bcvUot4e1UCrMU9P"
// );
/**
...
!(function () {
  ...
  process.versions.node
    ...
    : o && (i = self);
  var a =
      !i.JS_SHA3_NO_COMMON_JS && "object" == typeof module && module.exports,
  ...
  for (
    ...
    for (var m = k[x], E = m.bits, O = 0; O < E.length; ++O) {
      var z = m.name + "_" + E[O];
      if (
        (C.push(z), (S[z] = m.createMethod(E[O], m.padding)), "sha3" !== m.name)
      ) {
        var N = m.name + E[O];
        C.push(N), (S[N] = S[z]);
      }
  ...
  (t.prototype.update = function (t) {
    ...
    for (
      ...
      if (((this.lastByteIndex = i), i >= s)) {
        for (this.start = i - s, this.block = a[c], i = 0; i < c; ++i)
          d[i] ^= a[i];
        j(d), (this.reset = !0);
      } else this.start = i;
    }
  ...
  var j = function (t) {
    ...
    for (n = 0; n < 48; n += 2)
      (i = t[0] ^ t[10] ^ t[20] ^ t[30] ^ t[40]),
        ...
        (e =
          (d = t[8] ^ t[18] ^ t[28] ^ t[38] ^ t[48]) ^ ((a << 1) | (s >>> 31))),
        (r =
          (l = t[9] ^ t[19] ^ t[29] ^ t[39] ^ t[49]) ^ ((s << 1) | (a >>> 31))),
        (t[0] ^= e),
        (t[1] ^= r),
        (t[10] ^= e),
        (t[11] ^= r),
...
if (typeof overWriteHash !== "undefined") {
  tokenData.hash = overWriteHash;
}
...
console.log("overWriteHash: ", overWriteHash);

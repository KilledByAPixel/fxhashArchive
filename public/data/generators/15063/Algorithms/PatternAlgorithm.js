...
class RQVariables
{
	...
	// --------------------------------------------------
	is(n)
	{	return this.m_variables.hasOwnProperty(n);
	}
	M_get(n,d)
	{	return this.is(n)?this.m_variables[n].m_value:d??"";
	}

	M_getFloat(varname,d)
	{
		...
		{	
			...
			{	var a=d??{}
				for(var k in v.m_value) a[k] = parseFloat(v.m_value[k]);
				return a;
...
class PatternAlgorithm extends RQVariables
{
	...
	{	
		if(this.m_outputImage )
		{
			...
			{	this.M_log("paper = "+this.m_paperColor);
				if(this.m_bgTransparent || this.m_paperColor=="Transparent")
					this.m_outputImage.M_clear();
				else
					this.m_outputImage.M_fill( this.M_getPaperColor()); 
   ...
   // M_getClipParameters  
	...
	M_getDefaultStyle()
	{
		...
		this.m_bgTransparent = this.M_getBool("bgTransparent",false);
		this.m_allowLiveFillShape = this.M_getBool("isLiveFill",false);
	...
	{
		S??=this;v??=S;
		S.m_strokeColor = this.M_getColor(v.is("lineStyle")?v.M_getVarOption("paletteTag","color"):v.M_get("strokeColor",this.m_strokeColor));

		let sw,l=v.M_getFloat(sw="strokeWidth");
		S.m_strokeWidth=(l<=0 )?this.m_strokeWidth:Number(l)*this.upscale;
		S.m_isStrokeScale=v.M_getVarOption(sw,"scale");
		S.m_isStrkPrtct=v.M_getVarOption(sw,"prtct");
		S.m_paletteTag = v.M_get("paletteTag","line");
		S.m_paletteVariant = v.M_getVarOption("paletteTag","variant");
		let s=v.M_get("lineStyle","-");if(s&&s!='-') S.m_lineStyle=this.M_getLineStyle(s,v);
		S.m_isFill=v.M_getBool("isFill",false);
			
	}
	M_getLineStyle(name,v)
	{	let ls={name:name}
		switch(name)
		{ 	case 'demult':
			ls.ampl=v.M_getFloat("ampl",1)*this.upscale;
			break;
   ...
   {	if( S==undefined)
	...
	{	const reg=/rgba?\(([0-9\.]+),([0-9\.]+),([0-9\.]+),?([0-9\.]*)\)/
		let color = this.M_getPaperColor();
		if( color)
		{
   ...
   {	
	...
	M_getHatchFunctionsList()
	{
	 	return ["HatchShape2","TreeHatchShape","HatchFlowField","HatchCirclePacking","HatchCloudLines","PNGFill","HatchShape"];
	}
   ...
   M_hatchFuncLeaf(OBB,x,opts)
   {
	...
	M_applyDistortionsToSelect(selects,dSel)
	{	let ds=this.m_distortions,l=ds?ds.length:0;
		if(l)
		for(let is=0; is<selects.length; is++)  
		{ 	let sel=selects[is];
			for( let i=0; i<l; i++)
			{	let d=ds[i];
	   			if( !sel.querySelector(`option[value="${d.m_tag}"]`)) 
				{	let e=NewElt("option", {value:d.m_tag,text:d.m_tag,appendTo:sel});
					if(dSel==d.m_tag)
   ...
   {	let log=false;
	...
	M_readSpecialAddons(S,vars)
	{
		// "DistortionModifier"
		switch(vars.m_name)
		{	case 'DistortionModifier':
			{	// find the selector
				let d='distortion',dSel = vars.M_get(d),ds=this.m_distortions;
				// Find the corresponding select
				if(ds)
				{	let dName = vars.M_getVarName(d);
					// fill the UI
					let elts= document.querySelectorAll(`select[name=${dName}]`);
					if(elts.length)
						this.M_applyDistortionsToSelect(elts,dSel);
		
					// store the distortion in the object
					for(let i=0; i<ds.length;i++)
					{	//console.log(`Testing ${i} ${ds[i].m_tag} ${dSel}`);
						if(ds[i].m_tag==dSel)
							S.m_distortion=ds[i];
	...
	M_clearLog()
	{
		if(A.m_logObj)
		{
			...
			if(a && Array.isArray(a)) while(i=a.pop()) i.remove();
		}
	...
	M_computeLineMask(input,options)
	{
		...
		if( mask && (this.m_isUseMask || opts.active))
		{
			...
			let nbInp=isArr?input.length:1;
			...
			{	
				...
				{	let L=isArr?input[i]:input;
					let poly=L.m_isPolyLine;
					let T=RQMaths.M_getAABB(poly?L.m_points:[L.A,L.B]);
	...
	M_hatchShape(polyline,opts)
	{
		...
		while(D=distributionFunc.apply(this,[OBB,x,opts]))
		{
			...
			if( Array.isArray(L))
			{
				if(opts.lineStyle) L=this.M_lineStyle(opts.lineStyle,L);
...
M_drawInMask(shape,opts)
{
	if( shape)
	{
		...
		let ctx=this.m_mask.M_getContext();
...
M_drawLinesInMask(L,strokeWidth)
{
	...
	{	let out;
		switch(lst.name)
		{
			...
			{	out=Ls;
				...
				{ 	let L=Ls[j],l=0,n=L.M_nb(),ky0,pPrev;					
					for(let i=0;i<n;i++)
					{ let p=L.M_getPoint(i);
					  if(pPrev) l+=p.M_minus(pPrev).M_length();
					  pPrev=p;
					  ky0??=knx*(p.y+p.x)/this.H
					  if(noise.simplex2(knx*l/this.W,ky0)<0.2) p.penUp=1;
			...
			{	out=[];
				...
				{	let knx=30+ip*10;
					...
					for(let j=0;j<Ls.length;j++)
					{ 	let L=Ls[j],l=0,n=L.M_nb(),ky0,pPrev,t;					
						let L2=new RQPolyLine();
						for(let i=0;i<n;i++)
						{  let p=L.M_getPoint(i);
							if(pPrev){
							...
							let p2=p.clone();
							if(t)
							{	let rnd=noise.simplex2((knx+shf)*l/this.W,ky0);
								p2.M_add(t.x*amp*rnd,t.y*amp*rnd);
								//p2.M_add(rnd*10,Math.random()*10);
							}
							L2.M_addPoint(p2);
	...
	M_drawLines(group,lines,isMask,opt)
	{
		...
		if(group.m_lineStyle) Ls=this.M_lineStyle(group.m_lineStyle,Ls)
		if( this.m_outputFormat=="SVG")
		{
		...
		{	if( this.m_outputImage)
			{
				...
				else
				{
					...
					if( isMask)
						Ls= this.M_computeLineMask(Ls,opt);  
					let nb=Ls.length;
	...
	{	
		opt.lineStyle=group.m_lineStyle;
		if( this.m_outputFormat=="SVG")
		{

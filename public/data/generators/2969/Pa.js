...
class PatternAlgorithm extends RQVariables
{
	...
	M_createOutputCanvas()
	{
		...
		{	
			...
			this.M_log("M_createOutputCanvas");
			if( this.m_pngUpscale!=1)
				this.m_outputImage.M_getContext().scale(this.m_pngUpscale,this.m_pngUpscale);
			this.m_outputImage.M_getContext().save();
	...
	{	
		...
		if(O)
		{
			this.M_log("Doing M_initOutputImage for "+this.m_name+" with workarea="+this.m_workArea.M_getString()+" shape="+this.M_get("workareaShape"));
			...
			{	O.M_fill( this.M_getPaperColor(this.m_paperColor)); 
				...
				{	const shift=100;
					...
					//	ctx.putImageData(this.bgID,-shift+(parseInt(y/H*k)%shift),y);
						ctx.putImageData(this.bgID,-shift+parseInt(shift*Math.cos(y/O.m_height*k)),y);
				}


			
			}
			if( this.m_documentMargin)
			{
				ctx.beginPath();
				switch(this.M_get("workareaShape"))
				{
					default:
   ...
   // --------------------------------------------------
	...
	{
		let dv =document.createElement("div");dv.setAttribute("id",id);
		if(title)dv.innerHTML="<h4>"+title+"</h4>";
		dv.appendChild(content);
		if(out==undefined)
			out=document.getElementsByTagName('body')[0];
		if(out)
			out.appendChild(dv);
		return dv;
	}
	...
	{	this.M_log("M_showOutputImage");
		if( this.m_outputImage && this.m_outputImageDiv == undefined)
	   		//this.m_outputImage = this.M_createControlImg("outputImage","Output",this.m_outputImage.m_canvas,document.getElementById('ARTWORK'));
   ...
   // M_getClipParameters  
	M_getClipParameters()
	{
		this.M_log("M_getClipParameters");
	...
	M_getSizeMm()
	{
		...
		//$('#ARTWORK,#ARTWORK svg').css({width: ""+(this.m_sizeMm.width*148/98)+"mm"});
   ...
   {	if( S==undefined)
	...
	M_getPaperColor(colorName)
	{
		...
		if(colorName)
		{
			switch(colorName)
			{
				...
				case 'YellowWhite': color="rgba(225,226,207,1)"; break;
				case 'Watercolor': color="rgba(245,246,243,1)"; break;

				case 'Black': color="rgba(0,0,0,1)"; break;
				case 'Pink': color="rgba(214,96,134,1)"; break;

				case 'GreyGreen': color="rgba(158,182,154,1)"; break;
	...
	M_getPaperArrayRGBA()
	{
		...
		if( color)
		{
			let m = color.match(/rgba\(([0-9\.]+),([0-9\.]+),([0-9\.]+),1\)/);
			if(m && m.length==4)
			{
	...
	M_seed(seed,i)
	{
		if( i==undefined || i==0)
		{
			...
			this.random = fxrand; //sfc32(0x9E3779B9, 0x243F6A88, 0xB7E15162, seed);
			for (var i2 = 0; i2 < 15; i2++) this.random();
		}
		else
		{	
			this["random"+i]=fxrand;//sfc32(0x9E3779B9, 0x243F6A88, 0xB7E15162, seed);
   ...
   // ---------------------------------------------
   M_readParametricVariable(S,varname,scale,isSign) 
   {	
   		//console.log("M_readParametricVariable "+varname+" "+scale+" "+isSign);
		var val	= S["m_"+varname]  = S.M_getFloat(varname);
	   ...
	   //this.M_log("inputName="+inputName);
	   //var chk = $("<a class=\"btnShowVarOptions\">options</a>");
	   var o = null; //$("[name="+inputName+"Method]").parents("[group-name=MinMaxParameters]"); 
	   //this.M_log("group name="+inputName+"Method result="+group.length);
	   if( o )
	   {
	   		var me=this;
	...
	M_hatchFuncCloudLines(OBB, x ,opts)
	{
		...
		for( let x=0; x<dist; x+=step)
		{
			...
			decalX = 0; //opts.m_modulation.amplitude * Math.pow(Math.sin(x*angleFact*(1+rnd)),2);
			// x shift
			decalY = ampl*Math.max(noise.simplex2(Math.cos(angleNoise)*opts.m_modulation.noiseFact,Math.sin(angleNoise)*opts.m_modulation.noiseFact)-opts.m_groundCut/ampl,0);
		...
		if(isDrawInMask)
		{	let decalY = opts.m_groundCut-opts.m_strokeWidth;
   ...
   M_hatchFuncLeaf(OBB,x,opts)
   {
	...
	M_makeObjectSVG(id,W,H)
	{
		...
		let svg= document.createElementNS(_NS,"svg");
		...
		svg.setAttribute("zancanpatterns","http:www.zancan.fr/zancanpatterns.dtd");
		svg.setAttribute("version","1.1");
		svg.setAttribute("xmlns:xlink","http://www.w3.org/1999/xlink");
		svg.setAttribute("xml:space","preserve");
		svg.setAttribute("xmlns:serif","http://www.serif.com/");
		
		svg.setAttribute("width",`${(W/this.upscale)}mm`);
		svg.setAttribute("height",`${(H/this.upscale)}mm`);
		svg.setAttribute("viewBox",`0 0 ${W} ${H}`);
		svg.setAttribute("id",`${id}`);
		let svgElt=document.createElementNS(_NS,"div");
		document.getElementsByTagName("body")[0].appendChild(svgElt);
		return svg;
	...
	{ 	
		this.m_defaultSvgGroup = $("<g name=\""+name+"\" style=\""+style+"\" inkscape:groupmode=\"layer\" inkscape:label=\""+name+"\"></g>"); 
			...
			if(isInstanciate)
			{
				if( this.m_groups[tagName]==undefined)
				{
					...
					{	gr.m_active = true;
						...
						gElt.setAttribute("name",name);
						gElt.setAttribute("style",gr.style);
						gElt.setAttribute("inkscape:groupmode","layer");
						gElt.setAttribute("inkscape:label",name);
			...
			{	if( S.m_groups==undefined)
				...
				if( S.m_groups[Gr.name]==undefined)
				{
					...
					if(gp && this.m_isMakeSvgGroups)
					{
						...
						//gp.m_svgGroup = $("<g name=\""+name+"\" style=\""+gp.style+"\" inkscape:groupmode=\"layer\" inkscape:label=\""+name+"\"></g>");
						//A.svg.append(gp.m_svgGroup);
						let gElt = document.createElementNS(_NS,"g");
						gElt.setAttribute("name",name);
						gElt.setAttribute("style",gp.style);
						gElt.setAttribute("inkscape:groupmode","layer");
						gElt.setAttribute("inkscape:label",name);
						A.svg.appendChild(gElt);
						gp.m_svgGroup = gElt;
   ...
   {	return;
	   ...
	   let select=$(".SvgObjectEnum select");
	   let me=this;
	   select.each( function(i){
			let parents = $(this).parents(".templateClassItem");
			var tName="";
			var addAll = true; 
			...
			{	let Gr = D[i];
	   			...
	   			{	if( $(this).find("option[value=\""+Gr.name+"\"]").length==0) 
						   $(this).append("<option value=\""+Gr.name+"\">"+Gr.name+"</option>");			   			
				}
   ...
   M_readSvgGroupVariable(S, vars)
   {
	   ...
	   var selector = "select[name="+vars.M_getVarName('SvgObject')+"] option[value=\""+groupName+"\"]"; 
	   //$(selector).prop('selected', true);

	   if(  typeof A.m_groups==="object") 
		   ...
		   if(gr!=undefined)
		   {
			   ...
			   {	
					...
					this.M_getStyle(gr,vars);
				   	let style = this.M_getStyleAsString(gr);
	
				   //gr.m_svgGroup = $("<g name=\""+vars.M_get("SvgGroupId",gr.id) +"\" style=\""+style+"\" ></g>");
				   gr.m_svgGroup = document.createElementNS(_NS,"g");
				   gr.m_svgGroup.setAttribute("name",vars.M_get("SvgGroupId",gr.id));
				   gr.m_svgGroup.setAttribute("style",style);
				   A.svg.appendChild(gr.m_svgGroup);
				}
		   } 
	...
	M_readHatchVariable(S,vars)
	{
		...
		var selector = "select[name="+svgObjectName+"] option[value=\""+groupName+"\"]"; 
		//$(selector).prop('selected', true);
		if( S.m_groups==undefined)
			S.m_groups={};
		...
		{   let Gr = S.m_groups[groupName]; 
			...
			{	
				...
				if(groupActive)
				{
					...
					//gp.m_spacing = vars.M_getFloat("lineSpacing",{min:0.1,max:1});
				   ...
				   {   
					   ...
					   let style = this.M_getStyleAsString(gp);

					   gp.m_svgGroup = document.createElementNS(_NS,"g");
					   gp.m_svgGroup.setAttribute("name",vars.M_get("SvgGroupId",gp.id));
					   gp.m_svgGroup.setAttribute("style",style);
					   A.svg.appendChild(gp.m_svgGroup);	
					}
	...
	M_makeSvgGroupsCheckboxes()
	{
		...
		var p = $('#groupChks');
		if( p.length==0)
		{	p = $("<div class=\"PanelM\" id=\"groupChks\"></div>"); 
			p.insertAfter('#RQSitePanelArtwork .PanelM');
			$("<div class=\"RQPanelTitle\">SVG groups</div>").insertBefore(p);
		}		
		let Grs = A.m_groupsInstances;
		for( let ig=0; ig<Grs.length; ig++)
		{
			...
			if( group.m_active)
			{
			   let b = p.find("#"+groupBundleId)
			   {	if( !b.length)
				   {
					   b = $("<ul class=\"bundle\" id=\""+groupBundleId+"\"><div class=\"title\">"+group.bundle+"</div></ul");
					   p.append(b);
				   }
				...
				if( !b.find('#'+id).length)
				{
					...
					{	var chk = $("<input type=\"checkbox\"  id=\""+id+"\" checked=\"CHECKED\" value=\"on\"></input><label for=\""+id+"\">"+grpName+"</label>");
						var li=$("<li></li>");
						li.append(chk);
						let sw = group.m_strokeWidth?group.m_strokeWidth : A.m_strokeWidth;
						if( sw )
							li.append("<span class=\"stroke\">"+(sw/A.upscale)+"</span>");
						...
						chk.on("change",function(){
							if(A.svg)
							{
								if( group.m_svgGroup)
								{	if( $(this).is(":checked") )
										group.m_svgGroup.appendTo(A.svg) 
									else
										group.m_svgGroup.remove();
									
								}
								A.M_applyArtwork(A.M_getTagSVG(A.svg));
							}
						});
						b.append(li);
					}
	...
	M_log = (s,id) => new Promise((resolve, reject) => 
	{
		...
		if( !A.m_logObj)
		{
			A.m_logObj = document.getElementById("artworkLog");
			if( !A.m_logObj)
			{
				A.m_logObj =document.createElement("div");  A.m_logObj.setAttribute("id","artworkLog"); 
				let container = document.createElement("div"); container.setAttribute("id","logContainer");
				//let btnClear = $("<a class=\"btnClearLog\">Clear</a>");
				//btnClear.on("click",function(){ A.m_logObj.html("");});
				//container.append(btnClear);
				container.appendChild(A.m_logObj);

				Art.parentNode.insertBefore(container, Art.nextSibling);
			}
		}
		var o;
		if( id!=undefined && (A.m_logObj && (o=A.m_logObj.getElementById(id))))
		{
			o.innerHTML=s;
		}
		else 
	...
	M_computeLineMask(input,opts)
	{
		let outLines=[];
			if(input==null) return outLines;
		var multipleLines = input.m_isPolyLine;
		let bounds = (opts && opts.bounds) ? opts.bounds : this.m_workArea;
		let stopCollide = opts && opts.stopCollide; 
		...
		var mask = (opts && opts.mask)? opts.mask : this.m_mask;
		if( mask && (this.m_isUseMask || (opts && opts.active)))
		{
			var context = mask.M_getContext();
			var thres = 128;
			var thres3 = thres*thres*thres;
			let currentLine;
			var nbLines = 1;
			if( multipleLines)
			{	
			...
			for( let iLine = 0; iLine<nbLines; iLine++)
			{
				...
				{	let P=currentLine.A.clone();
					...
					{
						var isOut; 
						if( bounds.M_isPointInside(P) && this.m_workArea.M_isPointInside(P) )
						{
							if( data)
							{
								let ind = Math.round(P.x-x0)*4+ Math.round(P.y-y0)*rowBytes;
								isOut= (data[ind]*data[ind+1]*data[ind+2]>=thres3 || data[ind+3]<thres )?1:0;
							
							}
							else 
							{
								var p = context.getImageData(P.x,P.y, 1, 1).data;
								isOut= (p[0]*p[1]*p[2]>=thres3 || p[3]<thres )?1:0;
							}
						} else isOut = 1;
						if( wasOut==-1)
						{	wasOut=isOut;
	...
	M_defaultDistributionFunc(OBB,x,opts)
	{
		...
		{
			let hatchFunc = opts.hatchFunc? opts.hatchFunc : this.M_defaultHatchFunc;	
			var L = hatchFunc.apply(this, [OBB,x-OBB.w/2,opts] );

			let spacingFunc = opts.spacingFunc? opts.spacingFunc : this.M_defaultSpacingFunc;
	...
	M_hatchShape(polyline,opts)
	{
		...
		while(D=distributionFunc.apply(this,[OBB,x,opts]))
		{
			...
			{	
				...
				let Ls = this.M_computeLineMask( L , clipOpts);
	...
	M_putWorkareaInStack()
	{
		...
		this.M_log("M_putWorkareaInStack "+RQPrintR(o,1) );
	...
	M_drawLines(group,lines,isMask)
	{
		if( lines==undefined)
			return;
		...
		if( this.m_outputFormat=="SVG")
		{
			...
			{	
				if( isMask)
					group.m_lines.push( ...this.M_computeLineMask(lines));
				else
		...
		{	if( this.m_outputImage)
			{
				...
				else
				{
					...
					if( isMask)
						Ls= this.M_computeLineMask(lines);  
					let nb=Ls.length;
					for(let i=0; i<nb;i++)
					{
						let lines = Ls[i].M_getSVGPath(false); 
   ...
   {	
				...
				{			
					...
					switch(g.m)
					{
						...
						case 'workarea':
							...
							this.M_log("setting clip mask to "+g.type+" shape="+g.shape.M_getString());
							ctx.restore();
	...
	M_downloadPNGasSVG(filename)
	{
		...
		let svg=svgpng;// div.getElementsByTagNameNS(_NS,"SVG")[0];

		let paper = this.M_getColor("paper");

		let defs = document.createElementNS(_NS,"defs");
				...
				case 'fill':
					{
						if(g.l)
						{
							let elt = document.createElementNS(_NS,"path");
							elt.setAttribute("d",g.l);
							elt.setAttribute("style",`fill:${g.c===paper?"white":g.c}`);
							svgGrp.appendChild(elt);
						}
					}
					break;
				case 'workarea':
					switch(g.type)
					{
						...
						case "Circle":
							//ctx.rect(g.shape.x,g.shape.y,g.shape.w,g.shape.h);
							clipId++;
							let clip = document.createElementNS(_NS,"clipPath");
							clip.setAttribute("id",`_clip${clipId}`); //$(`<clipPath id="_clip${clipId}">${g.shape.M_toSVG(false)}</clipPath>`);
							clip.innerHtml=g.shape.M_toSVG(false);
							defs.appendChild(clip);
							svgGrp = document.createElementNS(_NS,"g");
							//svgGrp = document.createElementNS(_NS,"foreignObject");

							svgGrp.setAttribute("clip-path",`url(#_clip${clipId})`);
							svgGrp.setAttribute("id",`group${clipId}`);
							svg.appendChild(svgGrp);
							//svg.appendChild($(`<use clip-path="url(#clip${clipId})" href="#group${clipId}" />`).get(0));
							break;
					}
					//ctx.clip();
					break;
				...
				case 'stroke':
					{
						let elt = document.createElementNS(_NS,"path");
						elt.setAttribute("d" ,g.l);
						elt.setAttribute("stroke-width",g.w);
						elt.setAttribute("stroke",g.c===paper?"white":g.c);
	...
	M_getHTML()
	{
		/*var nbDots = this.M_getInt('nbDots');
		var svg= this.M_makeObjectSVG( "mainSVG",this.W,this.H );
		for( var i =0; i<nbDots; i++)
			svg.append("<circle cx=\""+RQMaths.random()*this.W+"\" cy=\""+RQMaths.random()*this.H+"\" r=\""+RQMaths.random()*10+"\" stroke-width=\""+this.m_strokeWidth+"\" stroke=\""+this.m_strokeColor+"\" fill=\"none\"></circle>");

		return this.M_getTagSVG(svg);*/
		return "TOTO";
	}
	M_init(isAutoRun)
	{
	...
	M_onAlgorithmDone()
	{
		...
		this.M_log("Algorithm "+this.m_name+" done.");
		if( this.m_outputFormat=="PNG" )
		{
